...
def loss_nonsaturating(g, d, x_real, *, device):
    ...
    real_loss = F.binary_cross_entropy_with_logits(d_xreal, torch.ones(batch_size, device=device)).mean()
    fake_loss = F.binary_cross_entropy_with_logits(d_xfake, torch.zeros(batch_size, device=device)).mean()
    d_loss = real_loss + fake_loss
    
    g_loss = -F.logsigmoid(d_xfake).mean()
    # YOUR CODE ENDS HERE
...
def conditional_loss_nonsaturating(g, d, x_real, y_real, *, device):
    ...
    # YOUR CODE STARTS HERE
    x_fake = g(z, y_fake)
    d_xreal = d(x_real, y_real)
    d_xfake = d(x_fake, y_fake)
     
    real_loss = F.logsigmoid(d_xreal).mean()
    fake_loss = F.logsigmoid(-d_xfake).mean()
    d_loss = -real_loss - fake_loss
...
def loss_wasserstein_gp(g, d, x_real, *, device):
    ...
    # You may find some or all of the below useful:
    #   - torch.rand
    #   - torch.autograd.grad(..., create_graph=True)batch_size = x_real.shape[0]
    x_fake = g(z)
    d_real = d(x_real)
    d_fake = d(x_fake)
